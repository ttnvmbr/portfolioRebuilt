...
function FrameFlipContainer(props) {
  return (
    ...
    >
      <FrameFlip />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center text-left w-[80%]">
        <div className="text-6xl">{props.icon}</div>
        <p className=" text-2xl p-4">{props.text}</p>
      </div>
    </motion.div>
  );
}
